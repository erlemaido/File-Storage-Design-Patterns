...
namespace WebApp.Controllers
{
    public class ProductsGeneratorController : Controller
    {
        ...
        public ProductsGeneratorController(AppDbContext context)
        {
            _context = context;
        }
        ...
        {
            var picturesBytes = new List<byte[]>();

            if (allPossiblePics != null && allPossiblePics.Any())
            {
                picturesBytes = allPossiblePics
                    .Select(picture => ConvertToBytes(picture).Result)
                    .ToList();
            }
            ...
            for (var i = 0; i < size; i++)
            {
                ...
                if (picturesBytes.Any())
                {
                    Console.WriteLine("ADDING pictures for product nr: " + i);
                    AddRandomProductPictures(picturesBytes, product);
        ...
        private static void AddRandomProductPictures(IReadOnlyList<byte[]> picturesBytes, Product product)
        {
            var productPictures = new List<ProductPicture>();

            var picturesBytesCount = picturesBytes.Count;
            var nrOfProductPictures = Random.Next(MinPicturesPerProduct,
                MaxPicturesPerProduct <= picturesBytesCount
                    ? MaxPicturesPerProduct + 1
                    : picturesBytesCount + 1);

            var unusedIndices = Enumerable.Range(0, picturesBytesCount).ToArray();

            for (var i = 0; i < nrOfProductPictures; i++)
            {
                var randomPictureIndex = GetRandomNumberFromArray(picturesBytesCount, unusedIndices);
                unusedIndices = unusedIndices.Where(x => !x.Equals(randomPictureIndex)).ToArray();
                var picture = picturesBytes[randomPictureIndex];
